...
///
/// Under `boxed-trait`, per-item futures are boxed so the join machinery
/// (`try_join_all`, `FuturesOrdered`, ...) is instantiated once for
/// `BoxFuture` instead of once per element type.
...
pub async fn resolve_list<'a, T: OutputType + 'a>(
    ...
    if !extensions.is_empty() {
        let mut futures: Vec<BoxFuture<'_, ServerResult<Value>>> =
            len.map(Vec::with_capacity).unwrap_or_default();
