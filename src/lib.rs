...
//! | **`boxed-trait`**              | Boxes resolver futures with [`async-trait`](https://crates.io/crates/async-trait) and dispatches resolution through trait objects, trading a little dynamic dispatch for far less monomorphization (smaller binaries, faster builds). |
...
//! | **`multipart`**                | Support [multipart GraphQL requests](https://github.com/jaydenseric/graphql-multipart-request-spec) (file uploads). Enabled by default; disable to drop `multer`/`encoding_rs` for faster builds. |
...
//! | **`regex`**                    | Enable the `regex` input validator. Enabled by default; disable to drop the [`regex` crate](https://crates.io/crates/regex) from the dependency tree for faster builds.                       |
