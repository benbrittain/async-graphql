...
| no_getters    | Skip generating the per-field async getter methods. Getters are only needed when the object is a member of an `Interface`; skipping them speeds up compilation of large schemas.                        | bool                                       | Y        |
